...
class GenerateSudokuPuzzle:
    ...
    def __init__(self, ctx: commands.Context, difficulty: str, grid: list[list[int]], num_of_zeros: int):
        self.counter = 0
        self.path = []
        self.ctx = ctx
    ...
    def solve_puzzle(self, grid) -> bool:
                ...
                for number in range(1, 10):
                    ...
                    if self.valid_location(grid, row, col, number):
                        ...
                        if not self.find_empty_square(grid):
                            self.counter += 1
                            if self.counter == self.num_of_zeros:
                                break
                        else:
    ...
    def generate_puzzle(self, grid) -> None:
        ...
        return self.remove_numbers_from_grid(grid)
...
class SudokuGame:
    ...
    def __init__(self, ctx: commands.Context):
        # self.grid = grid
        # self.puzzle = GenerateSudokuPuzzle.generate_puzzle()
        self.ctx = ctx
...
class SudokuCog(commands.Cog):
    ...
    def __init__(self, bot: Bot):
        ...
        # self.ctx = ctx
        # self.puzzle = GenerateSudokuPuzzle.generate_puzzle(puzzle)
    ...
    async def start(self, ctx: commands.Context) -> None:
        ...
        if self.games.get(ctx.author.id):
            ...
            return
        self.games[ctx.author.id] = SudokuGame(ctx)
...
class SudokuDifficulty(discord.ui.View):
    """A set of buttons for selecting the difficulty of the game."""

    def __init__(self, ctx: commands.Context):
        super(SudokuDifficulty, self).__init__(timeout=60)
        self.difficulty: str
        self.disabled = None
        self.ctx = ctx
        # self.children[0]

    @discord.ui.button(style=discord.ButtonStyle.green, label="Easy")
    async def easy_button(self, ctx: commands.Context, interaction: discord.Interaction, *_) -> None:
        """Button for selecting the "Easy" difficulty level."""
        # Generate 12 clues.
        await interaction.response.send_message("You chose the 'Easy' difficulty level.", ephemeral=True)
        difficulty = "easy"
        GenerateSudokuPuzzle.set_difficulty(ctx, difficulty)
        # return self.puzzle

    @discord.ui.button(style=discord.ButtonStyle.primary, label="Medium")
    async def medium_button(self, ctx: commands.Context, interaction: discord.Interaction, *_) -> None:
        """Button for selecting the "Medium" difficulty level."""
        # Generate 11 clues.
        await interaction.response.send_message("You chose the 'Medium' difficulty level.", ephemeral=True)
        difficulty = "medium"
        GenerateSudokuPuzzle.set_difficulty(ctx, difficulty)
        # return self.puzzle

    @discord.ui.button(style=discord.ButtonStyle.red, label="Hard")
    async def hard_button(self, ctx: commands.Context, interaction: discord.Interaction, *_) -> None:
        """Button for selecting the "Hard" difficulty level."""
        # Generate 10 clues.
        await interaction.response.send_message("You chose the 'Hard' difficulty level.", ephemeral=True)
        difficulty = "hard"
        GenerateSudokuPuzzle.set_difficulty(ctx, difficulty)
        # return self.puzzle
